...
static void usartSendData(u8 *sendData,u16 length)
{
	...
	for(i=0 ; i<length ; i++)
	{
		...
		while(USART_GetFlagStatus(USART1, USART_FLAG_TXE) == RESET);//等待串口的发送缓冲区为空，则可以继续发送数据
	}
}
...
void Init_Uart(u32 bound)
{
	...
	//引脚默认的功能为GPIO，作为串口的接受和发送引脚时需要端口复用
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource9,GPIO_AF_USART1); //GPIOA9复用为USART1
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource10,GPIO_AF_USART1); //GPIOA10复用为USART1
	
	//USART1端口配置
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9 | GPIO_Pin_10; //GPIOA9与GPIOA10
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//引脚设置成复用功能用于支持外设
