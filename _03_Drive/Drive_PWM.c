...
void PWM1_Init(u16 arr,u16 psc)
{
	...
	TIM_OCInitTypeDef  TIM_OCInitStructure;//输出比较通道，GPIO需要复用
	...
	TIM_OC1PreloadConfig(TIM1, TIM_OCPreload_Enable);  //使能TIM1在CCR1通道上的预装载寄存器
	TIM_OC2PreloadConfig(TIM1, TIM_OCPreload_Enable);  //使能TIM1在CCR2通道上的预装载寄存器
